      ...
      {focusedTasks.length === 0 ? (
        ...
        <div className="today-step__list" aria-label="今日焦点列表">
          {focusedTasks.map((task) => {
            ...
            const previewItems = task.checklist.slice(0, 2);
            const hiddenChecklistCount = Math.max(0, task.checklist.length - previewItems.length);
                ...
                {task.checklist.length > 0 ? (
                  <ul className="today-step-checklist" aria-label="清单预览">
                    ...
                    {hiddenChecklistCount > 0 ? (
                      <li className="today-step-checklist__more">还有 {hiddenChecklistCount} 项</li>
                    ) : null}
                  </ul>
                ) : null}
